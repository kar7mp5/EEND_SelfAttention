...
def _download_and_convert_audio(link: str, output_path: str, index: int):
    ...
    Args:
        ...
        info_file (str): Path to the info.txt file to store metadata.
    """
    try:
        yt = YouTube(link)
...
def youtube_to_audio(video_info: dict[str, list[str]], query: str):
    ...
    Args:
        links (list[str]): A list of YouTube video URLs to extract and convert to audio.
...
if __name__ == "__main__":
    query = "Tank moving sound"
    video_info = search_youtube_videos(query, 20)
    print(video_info)
    youtube_to_audio(video_info, query)
